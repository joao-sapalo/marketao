...
import { Truck, Plus, Search, Edit2, Trash2, Loader2, AlertTriangle, RefreshCw } from 'lucide-react';
...
export default function SupplierIndex() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await supplierService.getAll({ search, page, perPage: 10 });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await supplierService.delete(deleteModal);
      setDeleteModal(null);
      fetchData();
    } catch {
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="p-6 space-y-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-slate-400">
          ...
          <p className="text-sm">{search ? 'Tente outro termo de busca.' : 'Clique em "Novo Fornecedor" para adicionar.'}</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.map((supplier, i) => (
                  ...
                  >
                    <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{supplier.name}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{supplier.phone || '-'}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{supplier.email || '-'}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{supplier.nif || '-'}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{supplier.city || '-'}</td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => navigate(`/suppliers/${supplier.id}/edit`)} className="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => setDeleteModal(supplier.id)} className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </motion.tr>
                ))}
      ...
      {deleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-slate-800 rounded-xl shadow-xl p-6 max-w-sm w-full">
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-6">Tem certeza que deseja excluir este fornecedor?</p>
