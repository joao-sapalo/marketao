...
import { ArrowUpCircle, CheckCircle, Loader2, RefreshCw, AlertTriangle } from 'lucide-react';
import { createService } from '../../services/crudService';

const payableService = createService('accounts-payable');
...
export default function Payable() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await payableService.getAll(params);
  ...
  const handleMarkPaid = async (id) => {
    setPayingId(id);
    try {
      await payableService.update(id, { status: 'paid' });
      fetchData();
    } catch {
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data.map((item, i) => (
                  ...
                  >
                    <td className="px-4 py-3 font-medium text-slate-900 dark:text-white">{item.supplier?.name || item.supplier_name || '-'}</td>
