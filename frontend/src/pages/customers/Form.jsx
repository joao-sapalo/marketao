...
import { User, Phone, Mail, FileText, MapPin, Map, StickyNote, Save, Loader2, ArrowLeft } from 'lucide-react';
...
export default function CustomerForm() {
  ...
  const fields = [
    { name: 'name', label: 'Nome', icon: User, required: true, placeholder: 'Nome completo' },
    { name: 'phone', label: 'Telefone', icon: Phone, placeholder: '+244 900 000 000' },
    { name: 'email', label: 'Email', icon: Mail, type: 'email', placeholder: 'cliente@email.com' },
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="p-6">
      <button
        ...
        className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 mb-4 transition"
      >
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {success && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 rounded-lg px-4 py-3 mb-6 text-sm"
          >
            {success}
          </motion.div>
        )}
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-xl shadow-lg dark:shadow-slate-900/50 p-6 space-y-5">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition disabled:opacity-60"
            >
